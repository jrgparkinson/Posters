...
\node[anchor=north west] (fig2) at (0,0) {\includegraphics[width=1.0\linewidth, trim={0.0cm, 0.0cm, 0.0cm, 0.0cm}, clip]{figures/widdellFig1TaToFsTimeCropped.png} };

\end{tikzpicture}
\end{tikzfigure}
\end{adjustwidth}
\figureTitle{Fig. 1:} \figureCaption{\cite{Widell2006} observed intense salt fluxes $F_s$ below sea ice coincident with temperature changes in the atmosphere ($T_a$) and ocean ($T$).}
...
%        height=8em]
        \begin{tcolorbox}[boxsep=5mm, % internal spacing
width=\linewidth+0.9em,  nobeforeafter, 
sharp corners, rounded corners=north,
        colback=blue!5,  box align=top, 
        colframe=oxfordblue, 
        before skip=0.0em,
        left skip=-0.9em,
...
We consider 2-D simulations in a Hele-Shaw cell of width $0.5$m, height $1$m, and plate separation $d$. Water of initial salinity $S_0=30\,\mathrm{g}/ \mathrm{kg}$ and temperature $-2.8^\circ$C is frozen from above by applying a fixed atmospheric temperature $T_a = -10^\circ$C. We assume $K_0=10^{-9}\,\mathrm{m}^2$, and initially set $d=0.5$mm to restrict thermal convection in the underlying liquid. After $\sim0.5$m of ice growth, we increase $d$ to $2$mm and spin up simulations for 2 days before considering different warming scenarios.
...
\vspace{0.0em}
\figureTitle{Fig. 3}: Typical porosity $\chi$ in the sea ice, and salinity $S_l$ in the underlying  liquid, before warming is introduced. Remnant brine channels (narrow high porosity regions) are visible throughout the ice. Our domain is horizontally periodic, with an open bottom boundary and an impermeable, isothermal, top boundary.
\end{minipage} 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Case 1
\begin{tcolorbox}[width=0.5\linewidth+0.85em, nobeforeafter, box align=top, 
bottomrule=0pt, 
colframe=white, colback=white,  boxrule=0mm,
left skip=-1.0em,  left=0.0em, top=0.5em, right=0.5em, standard jigsaw, opacityback=0]
...
%\node[anchor=north west] (2d) at (0,0) {\includegraphics[ trim={0.0cm, 0.0cm, 0.0cm, 0.0cm}, clip]{figures/prePerturbationSmall.jpg}};

\node[anchor=north west] (diags) at (0,0) {\includegraphics[trim={0.0cm, 0.0cm, 0.0cm, 0.0cm}, clip]{figures/SurfaceWarmingDiagnostics.eps} };

\node[anchor=north west] (timeseries1) at ([xshift=0em,yshift=1em]diags.south west) {\includegraphics[trim={0.0cm, 0.0cm, 0.0cm, 0.0cm}, clip]{figures/SurfaceWarming-2TimeSeries.jpg}};

\node[anchor=south west] (timeseries2) at ([xshift=-1em,yshift=0]timeseries1.south east) {\includegraphics[trim={0.0cm, 0.0cm, 0.0cm, 0.0cm}, clip]{figures/SurfaceWarming-3TimeSeries.jpg}};

\node[anchor=north](a) at (timeseries1.north) {(a)};
\node[anchor=north](b) at (timeseries2.north) {(b)};
\end{tikzpicture}
\end{tikzfigure}
\end{minipage}\hfill
\begin{minipage}[t]{0.5\linewidth}
\vspace{1.3em}
%  atmospheric temperatures $T_a = -10^\circ$ C (i.e. unchanged), $-5^\circ$C, $-3^\circ$C, and $-2^\circ$C
\figureTitle{Fig. 4 ($\leftarrow$)}: Vertical fluxes of heat $F_h$ and salt $F_s$ at $z=0.3$m in response to increased atmospheric temperature (see legend).  Intense desalination events can be seen for $T_a=-3^\circ$C (at $t=$2 days) and $-2^\circ$C (at $t=4$ and $8$ days). After $\sim 15$ days the warming ice stops rejecting significant quantities salt. \\
\figureTitle{Fig. 5 ($\downarrow$)}: Vertical profiles for the simulation with (a) $T_a = -2^\circ$C and (b) $T_a = -3^\circ$C. Top panel: minimum vertical velocity, middle panel: change in horizontally average porosity, bottom panel: change in horizontally averaged bulk salinity. The sea ice interface is indicated by a white (top panel) or black (lower panels) solid line. Atmospheric warming increases the ice porosity, which allows salt to drain through the entire depth of ice. 
\end{minipage}
...
top=0.5em,  left=-0.2em, right=0.5em,
...
\node[anchor=north west] (diags) at (0,0) {\includegraphics[ trim={0.0cm, 0.0cm, 0.0cm, 0.0cm}, clip]{figures/BasalWarmingDiagnostics.eps} };

\node[anchor=north west] (timeseries1) at ([xshift=0em,yshift=1em]diags.south west) {\includegraphics[ trim={0.0cm, 0.0cm, 0.0cm, 0.0cm}, clip]{figures/BasalWarming-1TimeSeries.jpg}};

\node[anchor=south west] (timeseries2) at ([xshift=-1.6em,yshift=0]timeseries1.south east) {\includegraphics[ trim={0.0cm, 0.0cm, 0.0cm, 0.0cm}, clip]{figures/BasalWarming-1p5TimeSeries.jpg}};
...
\begin{minipage}[t]{0.48\linewidth}
\vspace{1.3em}
\figureTitle{Fig. 6 ($\leftarrow$)}:  Vertical heat $F_h$ and salt $F_s$ fluxes as in Fig. 4 but for simulations with an increased oceanic temperature $T_o$. Salt fluxes are typically positive due to the downward transport of fresh melt water. However, for $T_o>-2.5^\circ$C we observe brief intense negative salt fluxes. \\
\figureTitle{Fig. 7 ($\downarrow$)}:  Vertical profiles as in Fig. 5, but for oceanic warming with (a) $T_o=-1^\circ$C and (b) $T_o=-1.5^\circ$C.  An increase in porosity permits stronger convection through a deeper layer at the base of the ice. However a convectively stable layer of buoyant melt water forms below the ice, preventing rejected salt from mixing with the underlying ocean except in the case of intense desalination events.
\end{minipage}
...
%%\item \textbf{Basal melting typically prevents sa, though }
%\end{itemize}
%\end{minipage}
\end{tcolorbox}
